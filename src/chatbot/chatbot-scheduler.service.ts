...
export class ChatbotSchedulerService {
  ...
  async scheduleMorningCheckins() {
    ...
    try {
      const results = await this.emitterService.triggerMorningCheckins();
  ...
  // @Cron('0 13 * * 1-5') // 1:00 PM weekdays only
  async scheduleMiddayCheckins() {
    this.logger.log('Scheduled midday checkins starting...');
    try {
      const results = await this.emitterService.triggerMiddayCheckins();
  ...
  async scheduleEveningCheckins() {
    ...
    try {
      const results = await this.emitterService.triggerEveningCheckins();
  ...
  async scheduleWaterReminders() {
    ...
    try {
      const results = await this.emitterService.triggerWaterReminders();
  ...
  // @Cron('0 10,11,12,13,14,15,16,17 * * 1-5') // Every hour during work hours, weekdays  
  async scheduleScreenBreakReminders() {
    this.logger.log('Scheduled screen break reminders starting...');
    try {
      const results = await this.emitterService.triggerScreenBreakReminders();
  ...
  // @Cron('30 9,11,13,15,17 * * 1-5') // Every 90 minutes during work hours, weekdays
  async scheduleCodeBreakReminders() {
    this.logger.log('Scheduled code break reminders starting...');
    try {
      const results = await this.emitterService.triggerCodeBreakReminders();
  ...
  async scheduleCreativeDigestReminders() {
    ...
    try {
      const results = await this.emitterService.triggerCreativeDigestReminders();
  ...
  async schedulePostureCheckReminders() {
    ...
    try {
      const results = await this.emitterService.triggerPostureCheckReminders();
  ...
  async scheduleDeepBreathingReminders() {
    ...
    try {
      const results = await this.emitterService.triggerDeepBreathingReminders();
  ...
  async scheduleStretchBreakReminders() {
    ...
    try {
      const results = await this.emitterService.triggerStretchBreakReminders();
  ...
  async scheduleMentalHealthCheckins() {
    ...
    try {
      const results = await this.emitterService.triggerMentalHealthCheckins();
